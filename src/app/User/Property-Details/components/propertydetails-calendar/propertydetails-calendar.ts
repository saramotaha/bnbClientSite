...
mapAvailabilityDates(): void {
  ...
  for (const slot of this.propertyDetails.availabilityDates) {
    const dateStr = slot.date.split('T')[0]; // "yyyy-MM-dd"
    this.availableDatesMap.set(dateStr, slot.isAvailable);
  }
}
isAvailable(date: Date | null): boolean {
  if (!date) return false;
  const dateStr = date.toISOString().split('T')[0];
  return this.availableDatesMap.get(dateStr) ?? false;
}
